...
SHORTLINK_DOMAINS = {"bit.ly", "tinyurl.com", "t.co", "goo.gl", "is.gd", "buff.ly", "cutt.ly", "rebrand.ly", "shorturl.at", "linktr.ee", "beacons.ai", "taplink.cc", "bio.link", "direct.me", "msha.ke", "solo.to"}
SOCIAL_DOMAINS = {"discord.gg", "discord.com", "telegram.me", "t.me", "whatsapp.com", "wa.me", "facebook.com", "fb.com", "reddit.com", "twitter.com", "x.com", "instagram.com"}
APP_STORE_DOMAINS = {"play.google.com", "apps.apple.com", "itunes.apple.com", "appgallery.huawei.com"}
BUY_PATH_KEYWORDS = ["buy", "purchase", "pricing", "price", "order", "checkout", "pay", "plans", "subscribe", "subscription", "vip", "recharge", "topup", "payment"]
REFERRAL_TERMS = ["referral", "refer", "invite", "invitation", "affiliate", "partner", "commission", "ambassador", "sponsor", "sponsored", "promo", "promotion", "share"]
DISCOUNT_TERMS = ["discount", "coupon", "voucher", "promo code", "promocode", "code", "邀请码", "優惠碼", "优惠码", "折扣码", "折扣碼", "คูปอง", "โค้ด", "mã giảm", "kode", "cupom", "código"]
DISTRIBUTION_TERMS = ["reseller", "distributor", "dealer", "agent", "agency", "selling code", "redeem code", "device code", "code seller", "wholesale", "代购", "代購", "代理", "分销", "分銷", "销售", "銷售", "设备码", "設備碼", "兑换码", "兌換碼"]
CTA_TERMS = ["download", "buy", "purchase", "use my code", "use code", "click", "join", "contact", "get", "try", "sign up", "register", "subscribe", "top up", "recharge", "下载", "購買", "购买", "使用我的", "加入", "联系", "聯繫", "注册", "註冊"]
...
def url_contains_domain(url: str, domains: Sequence[str]) -> bool:
    ...
    return any(host == d.lower().strip() or host.endswith("." + d.lower().strip()) for d in domains if d)
...
def brand_alias_match(text: str, aliases: Sequence[str]) -> bool:
    text = text or ""
    return any(alias_regex(a).search(text) for a in aliases if a)


def brand_domain_match(text: str, domains: Sequence[str]) -> bool:
    urls = extract_urls(text or "")
    return any(url_contains_domain(u, domains) for u in urls)


def brand_match(item: Dict[str, Any], brand_cfg: Dict[str, Any]) -> bool:
    sn = item.get("snippet", {})
    text = f"{sn.get('title','')}\n{sn.get('description','')}"
    return brand_alias_match(text, brand_cfg.get("aliases", [])) or brand_domain_match(text, brand_cfg.get("official_domains", []))
...
def date_windows(after_iso: str, before_iso: str, days: int) -> List[Tuple[str, str]]:
    after, before = parse_iso(after_iso), parse_iso(before_iso)
...
def infer_scene(title: str, desc: str, scene_terms: Sequence[str]) -> str:
    ...
    for s in scene_terms:
        if s.lower() in t:
            scenes.append(s)
    for key, label in [("roblox", "Roblox"), ("seal m", "Seal M"), ("sailor piece", "Sailor Piece"), ("afk", "AFK"), ("cloud phone", "cloud phone")]:
        if key in t:
            scenes.append(label)
...
class YTClient:
    def __init__(self, api_key: str, sleep: float, retries: int, timeout: int):
        self.api_key, self.sleep, self.retries, self.timeout = api_key, sleep, retries, timeout
    ...
    def preflight(self) -> bool:
        return bool(self.get("videos", {"part": "id", "id": "dQw4w9WgXcQ"}).get("items"))
    ...
    def search_videos(self, q: str, max_results: int, after: str, before: str, region: Optional[str], lang: Optional[str], order: str) -> List[Dict[str, Any]]:
        ...
        while remaining > 0:
            data = self.get("search", {"part": "snippet", "type": "video", "q": q, "maxResults": min(50, remaining), "order": order, "publishedAfter": after, "publishedBefore": before, "regionCode": region, "relevanceLanguage": lang, "pageToken": token})
    ...
    def videos_list(self, ids: Sequence[str]) -> List[Dict[str, Any]]:
        ...
        for c in chunks(list(dict.fromkeys([x for x in ids if x])), 50):
            out.extend(self.get("videos", {"part": "snippet,statistics,contentDetails", "id": ",".join(c)}).get("items", []))
        return out

    def channels_list(self, ids: Sequence[str]) -> List[Dict[str, Any]]:
        out = []
        for c in chunks(list(dict.fromkeys([x for x in ids if x])), 50):
            out.extend(self.get("channels", {"part": "snippet,statistics,contentDetails", "id": ",".join(c)}).get("items", []))
        return out

    def channel_by_handle(self, handle: str) -> Optional[Dict[str, Any]]:
        items = self.get("channels", {"part": "snippet,statistics,contentDetails", "forHandle": normalize_handle(handle)}).get("items", [])
        return items[0] if items else None

    def playlist_items(self, playlist_id: str, max_results: int) -> List[Dict[str, Any]]:
        out, token, remaining = [], None, max_results
        while remaining > 0:
            data = self.get("playlistItems", {"part": "snippet,contentDetails", "playlistId": playlist_id, "maxResults": min(50, remaining), "pageToken": token})
            items = data.get("items", [])
            out.extend(items)
            remaining -= len(items)
            token = data.get("nextPageToken")
...
def analyze_conversion(title: str, desc: str, urls: Sequence[str], brand_domains: Sequence[str]) -> Dict[str, Any]:
    ...
    has_ref, has_disc = contains_any(text, REFERRAL_TERMS), contains_any(text, DISCOUNT_TERMS)
    ...
    if has_ref or has_disc or buy_urls:
        score = max(score, 3)
    landing = []
    if buy_urls: landing.append("buy_page")
    if app_urls: landing.append("app_store")
    if official_urls: landing.append("official_site")
    if social_urls: landing.append("social/private")
    if short_urls: landing.append("shortlink")
    if not landing and urls: landing.append("other_link")
    if not landing: landing.append("none")
    return {
        "has_any_link": bool(urls), "has_official_link": bool(official_urls), "has_buy_page": bool(buy_urls),
        "has_app_store_link": bool(app_urls), "has_referral_code": has_ref, "has_discount_code": has_disc,
        "has_shortlink": bool(short_urls), "has_social_link": bool(social_urls), "distribution_trace": has_dist,
        "landing_type": "; ".join(landing), "conversion_path_score": score,
        "official_urls": " | ".join(official_urls[:5]), "buy_urls": " | ".join(buy_urls[:5]),
        "app_store_urls": " | ".join(app_urls[:5]), "social_urls": " | ".join(social_urls[:5]),
        "short_urls": " | ".join(short_urls[:5]), "cta_text": find_lines_with_terms(desc, CTA_TERMS + REFERRAL_TERMS + DISCOUNT_TERMS + DISTRIBUTION_TERMS),
        "all_urls": " | ".join(urls[:20]), "url_count": len(urls),
    }
...
def build_video_row(item: Dict[str, Any], brand: str, queries: Sequence[str], channels: Dict[str, Dict[str, Any]], config: Dict[str, Any], official: Dict[str, Dict[str, Any]], run_time: dt.datetime) -> Dict[str, Any]:
    ...
    vid, ch_id = item.get("id"), sn.get("channelId", "")
    cm, off = channels.get(ch_id, {}), official.get(ch_id, {})
    is_off = ch_id in official
    title, desc, pub = sn.get("title", ""), sn.get("description", ""), sn.get("publishedAt", "")
    age = days_since(pub, run_time)
    views, likes, comments = to_int(st.get("viewCount")) or 0, to_int(st.get("likeCount")), to_int(st.get("commentCount"))
    return {
        "brand": brand, "video_id": vid, "video_url": video_url(vid), "title": title, "description": desc,
        "channel_id": ch_id, "channel_name": cm.get("title") or sn.get("channelTitle", ""), "channel_handle": cm.get("channel_handle", ""), "channel_url": channel_url(ch_id),
        "channel_subscribers": cm.get("subscriberCount"), "channel_video_count": cm.get("videoCount"), "channel_total_views": cm.get("viewCount"),
        "is_official_channel": is_off, "creator_type": "official" if is_off else "kol/creator",
        "official_config_name": off.get("official_config_name", ""), "official_config_handle": off.get("official_config_handle", ""),
        "publish_date": pub, "video_age_days": round(age, 3), "views": views, "likes": likes, "comments": comments,
        "views_per_day": round(views / max(age, 0.001), 6), "like_rate": round(likes / views, 8) if likes is not None and views else None,
        "comment_rate": round(comments / views, 8) if comments is not None and views else None,
        "duration_iso8601": cd.get("duration", ""), "query_keyword": " | ".join(sorted(set(queries))),
        "language_hint": language_hint(title + "\n" + desc), "region_guess": "", "game_scene": infer_scene(title, desc, config.get("scene_terms", [])),
        "content_type": infer_content_type(title, desc), "is_brand_video": True,
        "thumbnail_default": ((sn.get("thumbnails") or {}).get("default") or {}).get("url", ""), "collected_at_utc": iso_z(run_time),
    }
...
def collect_baselines(client: YTClient, video_rows: List[Dict[str, Any]], channels: Dict[str, Dict[str, Any]], brand_aliases: Dict[str, List[str]], baseline_count: int, run_time: dt.datetime) -> List[Dict[str, Any]]:
    ...
    all_aliases = [a for aliases in brand_aliases.values() for a in aliases]
    out = []
    for ch_id, rows in by_channel.items():
        ...
        for it in hydrated:
            vid, sn = it.get("id"), it.get("snippet", {})
            text = f"{sn.get('title','')}\n{sn.get('description','')}"
            if vid in target_ids or any(brand_alias_match(text, [a]) for a in all_aliases):
                continue
            st = it.get("statistics", {})
            views = to_int(st.get("viewCount")) or 0
            age = days_since(sn.get("publishedAt", ""), run_time)
            likes, comments = to_int(st.get("likeCount")), to_int(st.get("commentCount"))
            base.append({"views_per_day": views / max(age, 0.001), "like_rate": likes / views if likes is not None and views else None, "comment_rate": comments / views if comments is not None and views else None})
            if len(base) >= baseline_count: break
        avg_vpd, med_vpd = avg([b["views_per_day"] for b in base]), (sorted([b["views_per_day"] for b in base])[len(base)//2] if base else None)
        avg_like, avg_comment = avg([b["like_rate"] for b in base]), avg([b["comment_rate"] for b in base])
        for r in rows:
            perf = r["views_per_day"] / avg_vpd if avg_vpd else None
            out.append({
                "brand": r["brand"], "video_id": r["video_id"], "video_url": r["video_url"], "title": r["title"],
                "channel_id": ch_id, "channel_name": r["channel_name"], "channel_handle": r["channel_handle"], "channel_url": r["channel_url"], "creator_type": r["creator_type"],
                "subscriber_count": r.get("channel_subscribers"), "baseline_video_count": len(base),
                ...
                "brand_views_per_day": r["views_per_day"], "performance_index": round(perf, 6) if perf is not None else None,
                ...
                "baseline_status": "ok" if base else "no_baseline", "baseline_error": "",
            })
    return out
...
def summarize_brand(videos: List[Dict[str, Any]], conv_map: Dict[Tuple[str, str], Dict[str, Any]], baselines: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    out = []
    for brand in sorted({r["brand"] for r in videos}):
        rows = [r for r in videos if r["brand"] == brand]
        ...
        out.append({
            "brand": brand, "video_count": len(rows), "kol_video_count": sum(1 for r in rows if not r.get("is_official_channel")), "official_video_count": sum(1 for r in rows if r.get("is_official_channel")),
            "channel_count": len({r["channel_id"] for r in rows if r.get("channel_id")}), "kol_channel_count": len({r["channel_id"] for r in rows if r.get("channel_id") and not r.get("is_official_channel")}), "official_channel_count": len({r["channel_id"] for r in rows if r.get("channel_id") and r.get("is_official_channel")}),
            "total_views": sum(views), "avg_views": round(sum(views) / len(rows), 2) if rows else None, "median_views": median(views), "avg_views_per_day": round(avg([r.get("views_per_day") for r in rows]) or 0, 6) if rows else None,
            "link_rate": bool_rate(convs, "has_any_link"), "official_link_rate": bool_rate(convs, "has_official_link"), "buy_page_direct_rate": bool_rate(convs, "has_buy_page"), "app_store_rate": bool_rate(convs, "has_app_store_link"),
            "referral_code_rate": bool_rate(convs, "has_referral_code"), "discount_code_rate": bool_rate(convs, "has_discount_code"), "social_redirect_rate": bool_rate(convs, "has_social_link"), "shortlink_rate": bool_rate(convs, "has_shortlink"), "distribution_trace_rate": bool_rate(convs, "distribution_trace"),
            "avg_conversion_path_score": round(avg([c.get("conversion_path_score") for c in convs]) or 0, 4), "avg_performance_index": round(avg(perf) or 0, 6) if perf else None,
            "performance_index_gt_1_2_rate": round(sum(1 for p in perf if p > 1.2) / len(perf), 4) if perf else None, "performance_index_lt_0_8_rate": round(sum(1 for p in perf if p < 0.8) / len(perf), 4) if perf else None,
            "top_content_types": json.dumps(Counter(r.get("content_type") for r in rows).most_common(5), ensure_ascii=False), "top_scenes": json.dumps(Counter(r.get("game_scene") for r in rows).most_common(5), ensure_ascii=False),
        })
    return out
...
def summarize_creator_brand(videos: List[Dict[str, Any]], conv_map: Dict[Tuple[str, str], Dict[str, Any]], baselines: List[Dict[str, Any]], official_only: bool) -> List[Dict[str, Any]]:
    ...
    for (ch, brand), rows in sorted(grouped.items()):
        ...
        out.append({
            "brand": brand, "channel_id": ch, "channel_name": first.get("channel_name"), "channel_handle": first.get("channel_handle"), "channel_url": first.get("channel_url"),
            "creator_type": first.get("creator_type"), "is_official_channel": first.get("is_official_channel"), "official_config_name": first.get("official_config_name"), "official_config_handle": first.get("official_config_handle"),
            "channel_subscribers": first.get("channel_subscribers"), "channel_video_count": first.get("channel_video_count"), "channel_total_views": first.get("channel_total_views"),
            "video_count_for_brand": len(rows), "total_views_for_brand": sum(views), "avg_views_for_brand": round(sum(views) / len(rows), 2) if rows else None, "median_views_for_brand": median(views), "avg_views_per_day_for_brand": round(avg([r.get("views_per_day") for r in rows]) or 0, 6),
            "earliest_brand_video_date": iso_z(min(dates)) if dates else "", "latest_brand_video_date": iso_z(max(dates)) if dates else "", "avg_conversion_path_score": round(avg([c.get("conversion_path_score") for c in convs]) or 0, 4),
            "link_rate": bool_rate(convs, "has_any_link"), "buy_page_direct_rate": bool_rate(convs, "has_buy_page"), "referral_code_rate": bool_rate(convs, "has_referral_code"), "discount_code_rate": bool_rate(convs, "has_discount_code"), "social_redirect_rate": bool_rate(convs, "has_social_link"), "distribution_trace_rate": bool_rate(convs, "distribution_trace"),
            "avg_performance_index": round(avg(perf) or 0, 6) if perf else None, "brands_covered_by_this_channel_in_scope": " | ".join(covered), "brand_count_for_this_channel_in_scope": len(covered), "is_multi_brand_creator_in_scope": len(covered) >= 2,
            "top_content_types": json.dumps(Counter(r.get("content_type") for r in rows).most_common(5), ensure_ascii=False), "top_scenes": json.dumps(Counter(r.get("game_scene") for r in rows).most_common(5), ensure_ascii=False), "sample_video_urls": " | ".join(r.get("video_url", "") for r in rows[:5]),
        })
    return out


def write_outputs(outdir: Path, videos: List[Dict[str, Any]], conversions: List[Dict[str, Any]], baselines: List[Dict[str, Any]], brand_summary: List[Dict[str, Any]], creator_summary: List[Dict[str, Any]], official_summary: List[Dict[str, Any]], run_summary: Dict[str, Any]) -> None:
    ...
    files = {"videos.csv": videos, "kol_videos.csv": kol_videos, "official_videos.csv": official_videos, "conversion_paths.csv": conversions, "channel_baselines.csv": baselines, "brand_summary.csv": brand_summary, "creator_brand_summary.csv": creator_summary, "official_channel_summary.csv": official_summary}
    ...
    with pd.ExcelWriter(outdir / "youtube_kol_conversion_report.xlsx", engine="openpyxl") as writer:
        for sheet, rows in [("brand_summary", brand_summary), ("creator_brand_summary", creator_summary), ("official_channel_summary", official_summary), ("video_samples_all", videos), ("kol_videos", kol_videos), ("official_videos", official_videos), ("conversion_paths", conversions), ("channel_baselines", baselines), ("run_summary", [run_summary])]:
...
def main() -> int:
    ...
    summary: Dict[str, Any] = {"status": "started", "run_time_utc": iso_z(run_time), "config_file": str(args.config), "lookback_days": lookback, "published_after_effective": published_after, "published_before_effective": published_before, "public_data_only": True}
    try:
        ...
        if args.dry_run:
            summary["status"] = "dry_run_ok"; summary["estimated_quota_units"] = client.units
            save_json(outdir / "run_summary.json", summary); save_json(outdir / "api_call_log.json", client.calls); return 0
        ...
        official_items, official_brands, official_queries = collect_official_uploads(client, official, published_after, published_before, int(settings.get("official_uploads_max_per_channel", 500)), log)
        video_items.update(official_items)
        for vid, bs in official_brands.items(): vid_brands[vid].update(bs)
        for key, qs in official_queries.items(): vid_queries[key].update(qs)
        ...
        channels = {}
        for ch in client.channels_list(channel_ids):
            meta = channel_meta(ch); channels[meta["channel_id"]] = meta
        for ch_id, meta in official.items(): channels[ch_id] = {**channels.get(ch_id, {}), **meta}
        rows, conv_rows, conv_map = [], [], {}
        strict = bool(settings.get("strict_brand_filter", True))
        official_override = bool(settings.get("official_channel_brand_override", True))
        for vid, item in hydrated_by_id.items():
            ch_id = item.get("snippet", {}).get("channelId", "")
            if official_override and ch_id in official:
                brands_for_video = {official[ch_id].get("official_brand")}
                vid_queries[(vid, official[ch_id].get("official_brand"))].add("official_channel_brand_override")
            else:
                brands_for_video = set(vid_brands.get(vid, set()))
            for brand in sorted([b for b in brands_for_video if b]):
                if strict and ch_id not in official and not brand_match(item, brand_cfg.get(brand, {})):
                    continue
                ...
                conv.update({"brand": brand, "video_id": vid, "video_url": row["video_url"], "title": row["title"], "channel_id": row["channel_id"], "channel_name": row["channel_name"], "channel_handle": row["channel_handle"], "channel_url": row["channel_url"], "creator_type": row["creator_type"], "is_official_channel": row["is_official_channel"], "creator_relationship_type": relationship_type(row["is_official_channel"], conv, row["title"], row.get("description", ""))})
                rows.append(row); conv_rows.append(conv); conv_map[(vid, brand)] = conv
        ...
        summary.update({"status": "ok", "output_dir": str(outdir), "video_row_count": len(rows), "unique_video_count": len({r["video_id"] for r in rows}), "kol_video_rows": sum(1 for r in rows if not r["is_official_channel"]), "official_video_rows": sum(1 for r in rows if r["is_official_channel"]), "creator_brand_summary_rows": len(creator_summary), "official_channel_summary_rows": len(official_summary), "estimated_quota_units": client.units, "api_call_counts": dict(Counter(c["method"] for c in client.calls))})
        write_outputs(outdir, rows, conv_rows, baselines, brand_summary, creator_summary, official_summary, summary)
        save_json(outdir / "api_call_log.json", client.calls)
        log(f"Done: {outdir}")
        return 0
    except Exception as exc:
        summary.update({"status": "failed", "error": str(exc), "traceback": traceback.format_exc(), "estimated_quota_units": client.units})
        save_json(outdir / "run_summary.json", summary); save_json(outdir / "api_call_log.json", client.calls)
        with log_file.open("a", encoding="utf-8") as f: f.write("\n" + traceback.format_exc() + "\n")
        print(f"ERROR: {exc}")
        return 1
