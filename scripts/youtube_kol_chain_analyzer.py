...
def main() -> int:
    ...
    try:
        ...
        strict = bool(settings.get("strict_brand_filter", False))
        for vid, item in hydrated_by_id.items():
            for brand in sorted(vid_brands.get(vid, set())):
                sn = item.get("snippet", {})
                ch_id = sn.get("channelId", "")
                text = f"{sn.get('title','')}\n{sn.get('description','')}".lower()
                aliases = brand_cfg.get(brand, {}).get("aliases", [brand])
                if strict and ch_id not in official and not any(a.lower() in text for a in aliases):
                    continue
