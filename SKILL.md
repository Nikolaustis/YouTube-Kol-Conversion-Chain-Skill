...
RF_BAD_PATTERNS = [
    r"\bred\s*finger\b.*\b(color song|nursery|kids|baby|children|asmr|lemon|challenge|ring|makeup|horror|scp|garten|skibidi|game|games|chapter|walkthrough|ending|boss|level|apk|mod|gameplay)\b",
    r"\b(color song|nursery|kids|baby|children|asmr|lemon|challenge|ring|makeup|horror|scp|garten|skibidi|game|games|chapter|walkthrough|ending|boss|level|apk|mod|gameplay)\b.*\bred\s*finger\b",
]
...
def validate_brand(row: pd.Series, channel_role: str) -> Tuple[str, str]:
    ...
    if brand == "VS":
        # Separated "VS Phone" is not accepted by itself because it frequently
        # matches ordinary phone-comparison videos (PC vs phone, iPad vs phone, etc.).
        ...
        if re.search(r"\bvs\s+phone\b", text, flags=re.I):
            return "drop_false_positive", "ambiguous_vs_phone_not_accepted"
        return "drop_false_positive", "vs_brand_not_confirmed"

    if brand == "RF":
        # Drop non-cloud-phone meanings of Red Finger / RedFinger before accepting
        # any exact-looking title or hashtag. Spaced "Red Finger" is especially
        # noisy and is not accepted unless a target official domain is present.
        if "cloudemulator.net" in text or "redfinger.com" in text:
            return "keep", "redfinger_domain"
        if re_any(text, RF_BAD_PATTERNS):
            return "drop_false_positive", "red_finger_game_or_non_cloud_false_positive"
        if "redfinger" in text:
            if has_any(text, RF_STRONG_CONTEXT_TERMS):
                return "keep", "redfinger_exact_with_strong_context"
            return "drop_false_positive", "redfinger_without_cloud_phone_context"
        if re.search(r"\bred\s+finger\b", text, flags=re.I):
            return "drop_false_positive", "spaced_red_finger_ambiguous_not_accepted"
